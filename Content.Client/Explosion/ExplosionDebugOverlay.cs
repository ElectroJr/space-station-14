    ...
    public sealed class ExplosionDebugOverlay : Overlay
    {
        ...
        private void DrawScreen(OverlayDrawArgs args)
        {
            ...
            foreach (var epicenter in Tiles[1])
            {
                ...
                var box = Box2.UnitCentered.Translated(worldCenter);
                var topLeft = _eyeManager.WorldToScreen(box.TopLeft);
                var bottomLeft = _eyeManager.WorldToScreen(box.BottomLeft);
        ...
        private void DrawWorld(in OverlayDrawArgs args)
        {
            ...
            for (int i = 0; i < Tiles!.Count; i++)
            {
                ...
                foreach (var tile in Tiles[i])
                {
                    ...
                    var boundingBox = Box2.UnitCentered.Translated(center);
                    handle.DrawRect(boundingBox, color, false);
                    handle.DrawRect(boundingBox, colorTransparent);
                }
            }
        }
